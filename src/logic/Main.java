...
public class Main {
	 ...
	 * @param rows
	 * @param columns
	 * @param populationCordinates get valid cordinates from User Input and save
	 *                             them in populationCordinates
	 */
	private static String getStartingCordinates(Scanner myObj, int rows, int columns,
		...
		try {
			...
			{
				System.out.println(
						"Cordinates you entered are out of this Universe, so will not be considered populated");
			} else {
	...
	private static void playGame(int rows, int columns, ArrayList<Cordinate> populationCordinates,
	 ...
	 * @param rows
	 * @param columns
	 * @param universe based on the +ve and -ve values in universe we define the
	...
	private static void convertUniverseToNextgeneration(int rows, int columns, int[][] universe) {
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				...
				{
					if (universe[i][j] < 3) // Condition 1 (underpopulation for <2 neighbours) -> we checked with 3 as
											// we incremented starting from 1
					{
						universe[i][j] = 0;
	 ...
	 * @param rows
	 * @param columns
	 * @param universe We calculate live neighbours and put the values in place
	 *                 Increment +ve Values when Live and decrement -ve values for
	 *                 dead cells
	 */
